...
export async function generateRecipes(
    ...
    try {
        const response = await fetch(
            `${envVars.API_URL}/chatgpt/generate_recipes?amount=${amount}&constraints=${constraints}`
        );
        ...
        if (Array.isArray(data) && data.every((item) => typeof item === 'string')) {
            ...
            throw new Error(
                'Les données récupérées ne sont pas une liste de chaînes de caractères.'
            );
        }
    } catch (error) {
...
export async function describeRecipe(
    recipe: string,
    constraints: string
): Promise<APIresponse<DescribeRecipeType>> {
    try {
        const response = await fetch(
            `${envVars.API_URL}/chatgpt/describe_recipe?recipe=${recipe}&constraints=${constraints}`
        );

        if (!response.ok) {
            throw new Error(
                `Erreur lors de la récupération de la recette : ${response.statusText}`
            );
        }
        const data = await response.json();
